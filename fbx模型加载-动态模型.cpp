...
void C动态物体::加载网格(FbxNode* p网格节点)
{
    ...
    for (i = 0; i < l多边形数量; i++)
    {
        ...
        for (j = 0; j < l多边形顶点数量; j++)
        {
            ...
            顶点变换.m128_f32[0] = 控制顶点[l控制顶点索引].mData[0];
            顶点变换.m128_f32[1] = 控制顶点[l控制顶点索引].mData[1];
            顶点变换.m128_f32[2] = 控制顶点[l控制顶点索引].mData[2];
            ...
            case FbxGeometryElement::eByControlPoint:
                ...
                {
                case FbxGeometryElement::eDirect:
                    顶点变换.m128_f32[0] = p法向量->GetDirectArray().GetAt(l控制顶点索引).mData[0];
                    顶点变换.m128_f32[1] = p法向量->GetDirectArray().GetAt(l控制顶点索引).mData[1];
                    顶点变换.m128_f32[2] = p法向量->GetDirectArray().GetAt(l控制顶点索引).mData[2];
                ...
                case FbxGeometryElement::eIndexToDirect:
                    ...
                    顶点变换.m128_f32[0] = p法向量->GetDirectArray().GetAt(id).mData[0];
                    顶点变换.m128_f32[1] = p法向量->GetDirectArray().GetAt(id).mData[1];
                    顶点变换.m128_f32[2] = p法向量->GetDirectArray().GetAt(id).mData[2];
                    顶点变换 = XMVector3Transform(顶点变换, 旋转修正);
                    顶点.法向量.x = 顶点变换.m128_f32[0];
                    顶点.法向量.y = 顶点变换.m128_f32[1];
                    顶点.法向量.z = 顶点变换.m128_f32[2];
            ...
            case FbxGeometryElement::eByPolygonVertex:
                ...
                {
                case FbxGeometryElement::eDirect:
                    顶点变换.m128_f32[0] = p法向量->GetDirectArray().GetAt(l当前顶点计数).mData[0];
                    顶点变换.m128_f32[1] = p法向量->GetDirectArray().GetAt(l当前顶点计数).mData[1];
                    顶点变换.m128_f32[2] = p法向量->GetDirectArray().GetAt(l当前顶点计数).mData[2];
                    顶点变换 = XMVector3Transform(顶点变换, 旋转修正);
                    顶点.法向量.x = 顶点变换.m128_f32[0];
                    顶点.法向量.y = 顶点变换.m128_f32[1];
                    顶点.法向量.z = 顶点变换.m128_f32[2];
    ...
    旋转修正2 *= XMMatrixRotationAxis(XMVECTOR{ 0.0f, 1.0f, 0.0f, 0.0f }, -XM_PI / 2);
            ...
            for (m = 0; m < 动画轨道[j].帧[k].骨骼变换.size(); m++)
            {
                if (m骨骼名称[m] == 变换节点->节点名称)
                {
                    ...
                    while (p变换节点->父节点)
                    {
                        ...
                        旋转变换3 = XMMatrixInverse(0, 旋转变换3);

                        旋转变换 = 旋转变换3 * 旋转变换;
                        旋转变换 = XMMatrixInverse(0, 旋转变换);

                        最终变换 = 最终变换 * 旋转变换4 * 旋转变换;
